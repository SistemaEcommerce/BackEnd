...
$.ajax({
    url: "vistas/js/departamentos.json",
    type: "GET",
    cache: false,
    contentType: false,
    processData: false,
    ...
    success: function(respuesta) {
        ...
        respuesta.forEach(seleccionarRegion);

        function seleccionarRegion(item, index) {

            var region = item.name;
            var codRegion = item.code;

            if ($("#codigoRegion").val() == codRegion) {

                $("#regionSeleccionado").attr("value", codRegion);
                $("#regionSeleccionado").html(region);

            }

            $("#seleccionarRegion").append('<option value="' + codRegion + '">' + region + '</option>');
...
var envioLocal = $("#envioLocal").val();
var envioNacional = $("#envioNacional").val();
var tasaMinimaLocal = $("#tasaMinimaLocal").val();
var tasaMinimaNacional = $("#tasaMinimaNacional").val();
var seleccionarRegion = $("#codigoRegion").val();
...
$(".cambioInformacion").change(function() {
    ...
    envioLocal = $("#envioLocal").val();

    envioNacional = $("#envioNacional").val();

    tasaMinimaLocal = $("#tasaMinimaLocal").val();

    tasaMinimaNacional = $("#tasaMinimaNacional").val();

    seleccionarRegion = $("#seleccionarRegion").val();
...
function cambiarInformacion(modoPaypal, modoPayu) {
    ...
    datos.append("envioLocal", envioLocal);
    datos.append("envioNacional", envioNacional);
    datos.append("tasaMinimaLocal", tasaMinimaLocal);
    datos.append("tasaMinimaNacional", tasaMinimaNacional);
    datos.append("seleccionarRegion", seleccionarRegion);
